...
Qoder2API 是一个 Go 编写的 HTTP 服务器，它对 Qoder (qoder.sh) 编码智能体后端进行逆向工程，
并将其重新暴露为一个 **OpenAI 兼容**的 `/v1/chat/completions` 端点（同时支持流式 SSE 和
非流式 JSON）。CLI 工具 / 本地 LLM 客户端可以把请求指向这个桥接服务，而不必直接调用 OpenAI
API。
...
标准 Go 模块，仅使用标准库（无第三方依赖）。需要 Go 1.26+。

```bash
go build ./...          # 构建 cmd/qoder2api
go test ./...           # 运行各 internal 包的单元测试
go run ./cmd/qoder2api  # 启动服务器（需要 QODER_PAT）
```
...
若 `QODER_PAT` 未设置，启动时以 "Token required!" 退出。服务绑定
`http://<host>:<port>/v1/chat/completions`。`baseprompt.json` 在构建时通过 `//go:embed`
从 `internal/bridge/` 内嵌——运行时不依赖文件系统。
...
这是最密集的部分。消息转换与（已移除的）Java 逻辑一一对应：
...
- 工具调用根据 OpenAI 请求是否声明了 `tools` 分两条路径处理：
  ...
  - **未启用 tools**：Qoder 仍可能输出形似工具调用的文本；代码会识别 `"Tool calls:"` 文本
    ...
    解析失败则丢弃缓冲。
- `baseprompt.json` 模板定义了固定的 Qoder 请求形态（每条消息的 `response_meta` 含 token
  用量、`reasoning_content_signature`、`contents` 文本块等）。改动该结构通常意味着模板和
  构建代码要一起改。
- 流式处理（`StreamAccumulator`）会缓冲可能是工具调用前导的文本以便重新解析；首个 SSE chunk
  还会携带 `role`。
...
- **存在两种不同的日期方案，绝不能混淆**：job-token 客户端（`internal/auth`）和
  `signature.go` 使用 **RFC-1123 GMT**；Bearer/流式客户端（`bearerclient.go` 中的
  `SignedPostStream`）签名的是 **epoch 秒**的 `cosy-date`。混用会破坏上游鉴权——这与原始
  Java 一致：两个客户端本就使用不同的方案。
